...
class _ChatViewState extends State<ChatView> {
 ...
 @override
                                      ...
                                      child: Row(
                                        ...
                                        children: <Widget>[
                                          ...
                                          Padding(
                                            ...
                                            child: data[index].messageType == 'TEXT'
                                                ? Text(data[index].messageTextContent ?? '')
                                                : data[index].messageType =='AUDIO'
                                                    ? audioView(data[index])
                                                    : data[index].messageType == 'LOCATION'
                                                        ? locationView(data[index].locationChatMessage)
                                                        : data[index].messageType == 'CONTACT'
                                                            ? contactView(data[index].contactChatMessage)
                                                            : imageView(data[index].mediaChatMessage),
                                          ),
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: (_chatController.chatController.text.isNotEmpty)
                          ...
                          : InkWell(
                              onTap: () {
                                _chatController.isOnTapSend.value=true;
  ...
  Widget audioView(ChatMessageModel? chatMessageModel){
   return Obx(
         () => Row(
       children: [
         InkWell(
           onTap: () {
             ...
             !_mirrorFlyChatViewController
                 ...
                 .value;
             _mirrorFlyChatViewController
                 .audioPlay(chatMessageModel!);
           },
           child: !_mirrorFlyChatViewController
               .isPlayAudio
               .value
               ? AppImageAsset(
               image: ImageConstants
         ...
         AppText(
             _mirrorFlyChatViewController.durationToString(Duration(
                 milliseconds: chatMessageModel !=
                     null
                     ? chatMessageModel.mediaChatMessage
                     ?.mediaDuration ??
                     0
                     : 0)),
             fontSize: 14.px,
  ...
  Widget locationView(LocationChatMessage? locationChatMessage) {
    _mirrorFlyChatViewController.markerLocation.value =
        LatLng(locationChatMessage!.latitude, locationChatMessage!.longitude);
    ...
    return Container(
        ...
        child: Container(
          ...
          child: Obx(
            () => GoogleMap(
              markers: {_mirrorFlyChatViewController.marker.value},
              initialCameraPosition:
                  _mirrorFlyChatViewController.kGooglePlex.value,
              zoomControlsEnabled: false,
              zoomGesturesEnabled: false,
              onMapCreated: (GoogleMapController mapController) =>
                  _mirrorFlyChatViewController.onMapCreated(mapController),
  ...
  Widget imageView(MediaChatMessage? mediaChatMessage){
   return Container(
       margin: EdgeInsets
           .all(1.5.px),
       height: 200.px,
       ...
       child: Image(
         image: FileImage(
             File(mediaChatMessage?.mediaLocalStoragePath ??
                 '')),
         loadingBuilder:
             (context,
             child,
 ...
 Future<void> requestLocationPermission() async {
   ...
   final bool locationStatus= handlePermissionResult(status);
   if(locationStatus){
     await _mirrorFlyChatViewController
         .sendLocation();
   }
