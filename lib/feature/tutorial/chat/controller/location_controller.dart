...
class LocationController extends GetxController{
  ...
  onCameraMove(CameraPosition position){
    ...
    debugPrint('onCameraMove ${position.toString()}');
  }
  onTap(LatLng latLng){
    setLocation(latLng);
  }
  getLocation(){
    Geolocator.getLastKnownPosition().then((value){
      ...
      debugPrint('Location ${er.toString()}');
  ...
  Future<Placemark> getAddress(double lat,double lng)async{
    ...
    for (var element in addresses) {
      ...
      debugPrint('address-list ${element.toJson().toString()}');
    }

    debugPrint('address ${first.toJson().toString()}');
